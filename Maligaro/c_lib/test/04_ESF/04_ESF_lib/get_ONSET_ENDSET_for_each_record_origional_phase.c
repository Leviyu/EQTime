...
{
	fprintf(my_input->out_logfile,"---> get_ONSET_ENDSET_for_each_record Begin for origional EW ");
	int npts_phase;
	...
	for(ista = 0; ista < my_input->sta_num;ista++)
	{
		...
		{
			if( fabs( my_record[ista].ES_win[i] ) >= AMP )
			{
				AMP = fabs( my_record[ista].ES_win[i] );
				npts_peak = i;
			}
		}
		...
		{
			if(fabs( my_record[ista].ES_win[i] ) < fabs(noise_level) )
			{
				npts_ONSET = i;
				break;
			}
