...
set too_close_phase = $work_dir/too_close_phase_for_${PHASE}
/bin/rm  $too_close_phase > & /dev/null
...
foreach sta (`cat $ESinfo |awk '{print $1}'`)
##echo "---> Working on plotting $sta"
set too_close_flag = 0
...
# =========== for bp filter phase get bp_delay_time info =============
##if( $filter_flag == "bp" || $filter_flag == "lp" ) then
##if($PHASE == "Sdiff" || $PHASE == "SS" || $PHASE == "SSS" || $PHASE == "ScSScS" ) then
##set bp_delay_time = `awk 'NR==1 {print $1}' $work_dir/S_ES_bp_delay_time.out`
##set bp_delay_time = `printf "%.2f" $bp_delay_time`
##else 
##set bp_delay_time = 0
##endif
##else
...
set suf_phase = $PHASE
if($suf_phase == "SSSSm") then
	set suf_phase = SSSS
endif
if($suf_phase == "SSSSSm") then
	set suf_phase = SSSSS
endif
if($suf_phase == "ScSScSm") then
	set suf_phase = ScSScS
endif
if($suf_phase == "ScSScSScSm") then
	set suf_phase = ScSScSScS
endif
if($suf_phase == "ScSScSScSScSm") then
	set suf_phase = ScSScSScSScS
endif



set SURFACE_MIN = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph 4.5kmps -rel $suf_phase | awk 'NR==6 {print $11}'`
set SURFACE_MAX = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph 4.0kmps -rel $suf_phase | awk 'NR==6 {print $11}'`
set too_close_flag = `echo ddd | awk '{if('$SURFACE_MIN' < 5 && '$SURFACE_MAX' > -5 ) print 2;else print 0}'`
##echo "SURFACE $SURFACE_MIN $SURFACE_MAX"
if($too_close_flag == 2 ) then
echo $STA >> $too_close_phase
	endif




# ScS Add taup time info for phase ovelay for phase SS sS sSS
		if( $PHASE == "ScS" ) then
			set traffic_time_max = 5
			set traffic_phase_list = (S sS sSS SS )
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
				set prem_time = -1000
				endif
				set traffic_phase_array = ( $traffic_phase_array $prem_time )
				set prem_time_int = `printf "%.0f" $prem_time`
				if(`echo $prem_time_int|awk '{if($1>0) print $1; else print -1*$1}'` < $traffic_time_max) then
				set too_close_flag = 1
				echo $STA >> $too_close_phase
				endif
			end #phase_name 

		else if($PHASE == "S") then
			# calculate the Arrival Time of sS ScS
			set traffic_phase_list = (ScS sS SKS )
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
		...
		else if($PHASE == "Sdiff") then
			# calculate the Arrival Time of sS ScS
			set traffic_phase_list = (SKS )
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
		...
		else if($PHASE == "SS") then
			set traffic_time_max = 25
			set traffic_phase_list = ( ScS sScS   )
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
		...
		else if($PHASE == "SSS") then
			set traffic_time_max = 25
			set traffic_phase_list = (ScS sScS sSS )
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
		...
		else if($PHASE == "ScSScS" || $PHASE == "ScSScSm") then
			set traffic_time_max = 32
			set traffic_phase_list = (SS sSS  SSS )
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
		...
		else if($PHASE == "SSSS" || $PHASE == "SSSSm" && $PHASE == "SSSSS" || $PHASE == "SSSSSm") then
			set traffic_time_max = 40
			set traffic_phase_list = (  ScSScS  ScSScSScS sScSScS sScSScSScS)
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
		...
		else if($PHASE == "ScSScSScS" || $PHASE == "ScSScSScSm" || $PHASE == "ScSScSScSScS" || $PHASE == "ScSScSScSScSm") then
			set traffic_time_max = 40
			set traffic_phase_list = (SSSS SSSSS sSSSS sSSSSS)
			set traffic_phase_array = 
			foreach phase_name (`echo $traffic_phase_list`)
				set prem_time = `taup_time -mod prem -h $EQ_dep -deg $DIST -ph $phase_name -rel $PHASE |awk 'NR==6 {print $11}'`
				if($prem_time == "" ) then
...
set traffic_color = (green purple orange 104/178/255 )
...
EOF
		...
		#set the color of emp if quality bad plot yellow emp
		if($too_close_flag == 1) then
			set emp_color = blue
		else if ($too_close_flag == 2) then
			set emp_color = green
		else
			set emp_color = red 
		endif
		...
		psxy $gau -JX -R -W/purple -O -K>>$OUTFILE
...
set i = 1
foreach phase_name (`echo $traffic_phase_list`)
	set prem_time = ${traffic_phase_array[${i}]}
	set color = ${traffic_color[${i}]}
		psxy -JX -R -Wfaint/${color}ta -O -K <<EOF>>$OUTFILE
$prem_time -1
$prem_time 1
EOF
##echo "add traffic for $PHASE for phase $phase_name premtime $prem_time color ${traffic_color[${i}]}"
@ i ++
	if($i > $#traffic_phase_list ) then
	break
	endif
	end #phase_name 
...
$SHELL_DIR/find_mid_point_on_sphere <<EOF >>MID
