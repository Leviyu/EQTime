...
set C_DIR = $6
set DIR = $7
set OUT = $PLOTDIR/${EQ}_ES_updated.ps
set OUT_pdf = $PLOTDIR/${EQ}_ES_updated.pdf
...
foreach PHASE ( `cat $PHASE_LIST`)
	...
	set ES = $work_dir/${EQ}/${EQ}.${PHASE}.${COMP}.new_ES
	set STD = $work_dir/${EQ}/${EQ}.${PHASE}.${COMP}.new_ES_STD
	set S_ES = $work_dir/${EQ}/${EQ}.S.${COMP}.new_ES
	...
	set xmin = $tmp[1]
	set xmax = $tmp[2]
echo "FOr $PHASE xminmax is $xmin $xmax"
