...
pstext -JX6i/1i -R0/10/0/10 -Y9.5i -K -N -P<< EOF >> $OUT
...
psxy -R$RANGE -J$SCALE -Ba10f5:"Seconds (Relative to PREM prediction)":/a${Y_INC}f${Y_FLAG}WSne:"Distance (Degree)": -Y-1i -O -K -N  <<EOF>> $OUT
EOF
...
psxy $first_EW_STD -G192/192/192 -L -JX1.0i/0.4i -R$first_EW_xmin/$first_EW_xmax/-1.1/1.1 -Y-1i -O -K -N  << EOF >> $OUT
EOF
psxy $first_EW -W/red -JX -R$first_EW_xmin/$first_EW_xmax/-1.1/1.1  -O -K -N  << EOF >> $OUT
EOF
pstext -J -R -N -O -K << EOF >> $OUT
$first_EW_xmin -1.2 5 0 0 LB E.W.(0)
EOF


psxy -JX -R -O -K -Y1i << EOF >> $OUT
EOF
...
set first_EW_xmin = `cat $first_EW|minmax -C |awk '{print $1}'`
set first_EW_xmax = `cat $first_EW|minmax -C |awk '{print $2}'`
psxy $first_EW_STD -G192/192/192 -L -JX1i/0.4i -R$first_EW_xmin/$first_EW_xmax/-1.1/1.1 -Y-1i -O -K -N  << EOF >> $OUT
EOF
psxy $first_EW -W/red -JX -R$first_EW_xmin/$first_EW_xmax/-1.1/1.1  -O -K -N  << EOF >> $OUT
EOF
pstext -J -R -N -O -K << EOF >> $OUT
