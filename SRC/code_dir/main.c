...
int main(int argc, char* argv[])
{
	...
	fprintf(my_input.out_logfile,"---> Working on Reading in info, EQ: %s PHASE %s delta %lf \n", EQ_name, PHASE_name, delta);

	read_input_info(&my_input);
	my_input.sta_num =  count_file_num(my_input.event_file);
	fprintf(my_input.out_logfile,"Total record number is %d \n",my_input.sta_num);
	...
	output_long_phase_window_distsum(my_record,&my_input);
