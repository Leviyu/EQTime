...
data = pd.read_csv(inputfile,delimiter=r'\s+',names = header.columns,skipinitialspace=True,usecols=np.arange(54))
#print(data.head().T)
...
NET_LIST = pd.read_csv("./NET_LIST.csv")
unseen_net_filter = [x not in NET_LIST['NET'].values for x in data['NET']] 
#print(unseen_net_filter)
data.loc[unseen_net_filter,'NET'] = '3C'
...
for feature in categorial_feature:
    ...
    # save encoder
    label.classes_ = np.load(feature+'.encoder.npy',allow_pickle=True)
...
#print(data[all_feat].T)

data[all_feat].to_csv("what.csv",index=False)

all_feat.remove('ONE_PERIOD')
all_feat.remove("SNR_PEAK")

for model_num in model_list:
    model = pickle.load(open("./lightgbm_EW."+str(model_num)+".pkl",'rb'))
    pred = model.predict(data[all_feat],num_iteration=1000)
    #final_pred += np.array(pred) / len(model_list)
    final_pred_int['pred'] += np.array( pred) / len(model_list)
    #print(pred)
    #print([int(x/0.5) for x in pred])

#print(final_pred_int.iloc[0].T)
prob = final_pred_int['pred'].copy()
...
# use the raw probability too
final_pred_int['prob'] = prob
